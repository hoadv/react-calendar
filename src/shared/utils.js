import { getRange, getDay, getMonthName, getYear, getDayName } from './dates';
...
export function isCurrentDate(date, dateType) {
  const now = new Date();
  const dateRange = date instanceof Array ? date : getRange(dateType, date);

  return isValueWithinRange(now, dateRange);
}

export function getCurrentDate() {
  const now = new Date();

  return `${getDayName(now)} ${getMonthName(now)} ${getDay(now)}, ${getYear(now)}`;
}
